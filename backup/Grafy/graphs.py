    ...
    for fixed_val in fixed_values:
        ...
        for key, (fps, mean_error, median_error, precision, recall) in CONFIG_RESULTS.items():
            ...
            if (
                ...
                plt.scatter(fps, median_error, label=f"{shape:<7} {color:<7}", 
        ...
        plt.legend(title=f"Shapes ({shapes}) \n Colors ({colors})", fontsize=14, title_fontsize=18, loc='best', prop={'family': 'monospace', 'size': 17})
